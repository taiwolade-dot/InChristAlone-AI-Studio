...
def api_session_state(session_id):
    ...
    if questions:
        idx = quiz_session.current_question_index
        if idx < len(questions):
            question = questions[idx]
            seconds_left = quiz_session.seconds_per_question

            if quiz_session.status == 'active' and quiz_session.question_started_at:
                import datetime
                now = datetime.datetime.utcnow()
                elapsed = int((now - quiz_session.question_started_at).total_seconds())
                seconds_left = max(0, quiz_session.seconds_per_question - elapsed)
...
def api_submit_answer(session_id):
    ...
    return jsonify({'is_correct': is_correct, 'correct_index': question.correct_index})
